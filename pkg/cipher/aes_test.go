...
const (
	KEY_16 string = "0123456789abcdef"
	KEY_24 string = "0123456789abcdef01234567"
	KEY_32 string = "0123456789abcdef0123456789abcdef"
)

func TestAESCipher(t *testing.T) {
	hexData, _ := hex.DecodeString("130913f")
	ac, _ := NewAESCipher(KEY_16)
...
func TestNewAESCipher(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "16",
			args:    args{key: KEY_16},
			want:    &AESCipher{key: KEY_16},
			wantErr: false,
		},
		{
			name:    "24",
			args:    args{key: KEY_24},
			want:    &AESCipher{key: KEY_24},
			wantErr: false,
		},
		{
			name:    "32",
			args:    args{key: KEY_32},
			want:    &AESCipher{key: KEY_32},
			wantErr: false,
		},
		{
